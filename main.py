...
app = ApplicationBuilder().token("5452169338:AAGCq9zOWxcBz_YNp73F4dV4JIxDDfWT7Dc").concurrent_updates(True).build()

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("use /help for further help", quote=True)

async def edmx(update: Update, context):
    try:
...
<code>--key {keys}</code>        
    ...
    result = await down(v=v, a=a, isEdmx=True)
    if result == 'OK':
